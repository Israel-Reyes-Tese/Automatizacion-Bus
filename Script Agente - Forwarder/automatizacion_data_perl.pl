...
sub propiedades_agente {
    ...
    unless (-d $ruta_agente) {
        ...
        return \%propiedades;  # Retornar propiedades vacías en caso de error
    }
    ...
    if (-d "$ruta_agente/ABR" && -f "$ruta_agente/ABR/SNMPAgente.pm") {
        $tipo_agente = 'SNMP';
        # Si aún no se encontró IP y PUERTO, buscar en SNMPAgente.pm
    ...
    $propiedades{'SEVERITY'} = $severity;
    $propiedades{'Tipo de Agente'} = $tipo_agente;
    # Validar si IP y PUERTO son válidos
...
sub extraer_ip_puerto_principal {
    ...
    closedir($dir);
    # Seleccionar el archivo más parecido al nombre del agente
    
    my $archivo_principal = (sort { length($a) <=> length($b) } @archivos_principales)[0];
    unless ($archivo_principal) {
        warn "No se encontró un archivo principal Perl en '$ruta_agente'";
        return ($ip, $puerto);
    }
...
sub extraer_ip_puerto_config {
    ...
    # Si no se encontraron archivos en la carpeta config, buscar en la raíz
    unless (@archivos_config) {
        warn "No se encontraron archivos de configuracion en '$config_dir', buscando en la raíz del agente.";
        opendir(my $dir, $ruta_agente) or do {
            warn "No se pudo abrir el directorio del agente '$ruta_agente': $!";
            return ('Desconocida', 'Desconocido', 'No se encontraron archivos de configuracion');
        };
        # Filtrar archivos de configuracion config solamente los que tengan extensión .abr
        @archivos_config = grep { /^config.*\.abr$/i } readdir($dir);
        @buscar_raiz = 1;
        closedir($dir);

        unless (@archivos_config) {
            warn "No se encontraron archivos de configuracion en la raíz del agente '$ruta_agente'";
            return ('Desconocida', 'Desconocido', 'No se encontraron archivos de configuracion');
        }
    }
    ...
    foreach my $archivo_config (@archivos_config) {
        ...
        if (open my $fh, '<', $ruta_archivo_config) {
            while (my $line = <$fh>) {
                chomp $line;
                next if $line =~ /^\s*#/;  # Ignorar líneas comentadas
...
sub obtener_servicios {
    ...
    while (my $entrada = readdir($dir)) {
        ...
        if ($entrada =~ /^agt_(.*)\.service$/) {
            ...
            # Nombre completo del servicio con todo y extensión
...
# Unir los datos de los agentes y sus propiedades - tomando como referencia el nombre del agente y en caso de no existir propiedades, se asigna un hash vacío
...
my $agentes_info_con_servicio = { 
    map { 
        ...
        # Si no se encontraron servicios, intentar con el nombre en mayúsculas
        if (!@servicios) {
        ...
        # Ultima comprobación si no se encontraron servicios
        if (!@servicios) {
            ...
            foreach my $servicio (map { @$_ } values %$servicios_data) {
                ...
                # Valudar si no contiene datos cambiar el nombre a minúsculas
                ...
                # Validar si no se encontraron servicios para el agente cambiar el nombre a minúsculas
                if (!@servicios) {
        ...
        if ($nombre_agente eq 'CISCOPRIME_AGT_PRI' || $nombre_agente eq 'ciscoprime_agt_pri' || $nombre_agente eq 'ciscoprime') {
            ...
            #print "Servicios disponibles para el agente '$nombre_agente':\n";
            #print "Servicios data: ", Dumper($servicios_data), "\n";
        }
...
sub anexar_info_forwarder {
    ...
    foreach my $agente (keys %$agentes_info_con_servicio) {
        ...
        if ($agente eq 'TEKELEC_REPORTES' || $agente eq 'tekelec_reportes') {
            print "Servicios disponibles para el agente '$agente':\n";
...
# Fusionar la información de FILIAL.csv con agentes_info_con_servicio
...
# Añexar la información de FILIAL.csv a los agentes
...
print "Ingrese la ruta donde desea guardar el archivo CSV (deje en blanco para usar la ruta por defecto): ";
chomp(my $ruta_csv = <STDIN>);
$ruta_csv ||= 'agentes_info_con_servicio.csv';
